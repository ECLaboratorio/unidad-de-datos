...
datos_pcr <- datos_provincias %>% 
    select(fecha, iso, provincia, poblacion, pcr=num_casos_prueba_pcr, otras_pruebas=num_casos_prueba_otras)
...
datos_pcr_espana <- datos_pcr %>% 
    ...
    summarise(poblacion=sum(poblacion),
              pcr=sum(pcr),
              otras_pruebas=sum(otras_pruebas)) %>% 
    mutate(idpro="ES",
           provincia="España",
           suma_pruebas=pcr+otras_pruebas,
    ...
    filter(fecha>="2020-02-24" & fecha <= "2020-12-22") %>% 
